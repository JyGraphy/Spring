...
// 스프링 4에서는 exenteds HandlerInterceptorAdapter
// 스프링 5에서는 implements HandlerInterceptor

// HandlerInterceptor는 인터페이스이지만, 내부함수가 default 키워드로 처리되어 있다
// 인터페이스에서 default 함수는 바디를 가질 수 있다
// 미구현된 메서드만 구현하면 된다

// 추상 메서드가 아니라서 모든 메서드를 의무적으로 오버라이딩 할 필요가 없다
...
public class LoginInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// 반환 값에 따라 예정된 내용을  그대로 처리할지, 다른 내용을 수행할지 결정한다
		...
		if(session.getAttribute("login") == null) {		// 만약 세션에 로그인이 없으면
			...
			System.out.println("로그인 상태가 아니므로 로그인 페이지로 이동\n");
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		// header.jsp를 사용할 수 도 있으나, 모든 페이지에 영향을 주기 때문에 일부 페이지에만 적용하기 쉽지 않다
