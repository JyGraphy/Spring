...
public class UploadService {
	...
	public UploadService() {
		// 스프링 빈이 생성될때 (=서버가 시작될 때) saveDirectory  경로를 체크라고 없으면 새로 만든다
...
//		System.out.println(ext);
		...
		// 새로 저장될 이름은 중복되지 않도록 UUID를 사용한다 -> 그래야 같은 이름의 파일이라도 붙여넣지 않고 각각의 파일로 생성이 된다
...
//		System.out.println(storedFileName);
		...
		try {
			dto.getUpload().transferTo(f);					//transferTo 처리를 하면  실제 파일이 생성되는 함수를 생성
			// 서버 메모리상에 있는 MultipartFile -> 지정한 위치의 저장장치에 저장 java.io.File
